...
from poisson_neur import PoissonNeuron
from strdp_synapse import STRDP_Synapse
import numpy as np

class Network:
    def __init__(self, Tmax, scale, n_pre, n_post, pre_rates, weight_matrix):
        self.Tmax = Tmax
        self.scale = scale
        self.n_pre = n_pre
        self.n_post = n_post
        self.pre_rates = pre_rates

        # time
        self.T = np.linspace(0, Tmax, Tmax * scale)
        self.dt = self.T[1] - self.T[0]

        # neurons: pre - PoissonNeuron, post - LIF_neuron
        self.pre_neurons = np.array([
            PoissonNeuron(rate=pre_rates[i], dt=self.dt, type='pre') 
            for i in range(n_pre)
        ], dtype=object)
        
        self.post_neurons = np.array([
            LIF_neuron(type='post', dt=self.dt) for _ in range(n_post)
        ], dtype=object)
